...
function accountRewardsCtaCard(input: { rewardsCtaEnabled?: boolean }) {
  const body = input.rewardsCtaEnabled
    ? "Create your GameDayGrabs account to track orders and rewards. Earn points on eligible purchases. Redemption coming soon."
...
export function buildOrderConfirmationEmail(input: OrderConfirmationEmailInput): StorefrontRenderedEmail {
  ...
  const text = [
    ...
    input.accountCtaEnabled
      ...
      : null,
    input.accountCtaEnabled && input.rewardsCtaEnabled ? "Earn points on eligible purchases. Redemption coming soon." : null,
