...
function inventorySaleToDTO(
  ...
  const originalCharged = sale.totalCents === null ? sale.grossSale : centsToMoney(sale.totalCents);
  const refundedAmount = roundPosMoney(Math.min(Math.max(0, sale.refundedAmount ?? 0), Math.max(0, originalCharged)));
  const refundedTax = sale.refundedTaxCents === null ? 0 : centsToMoney(sale.refundedTaxCents);
  const refundedMerchandise = roundPosMoney(Math.max(0, refundedAmount - refundedTax));
  const saleStatus: InventorySaleDTO["saleStatus"] =
    refundedAmount >= originalCharged && refundedAmount > 0
      ? "refunded"
      : refundedAmount > 0
        ? "partially_refunded"
        : "active";
  const activeGrossSale = saleStatus === "refunded" ? 0 : roundPosMoney(Math.max(0, sale.grossSale - refundedMerchandise));
  const activeNetSale = saleStatus === "refunded" ? 0 : roundPosMoney(Math.max(0, sale.netSale - refundedMerchandise));
  ...
  return {
    ...
    taxableSubtotalCents: sale.taxableSubtotalCents,
    taxCents: sale.taxCents,
    totalCents: sale.totalCents,
    taxProvider: sale.taxProvider,
    stateTaxCents: sale.stateTaxCents,
    countySurtaxCents: sale.countySurtaxCents,
    combinedRateBasisPoints: sale.combinedRateBasisPoints,
    taxJurisdictionCountry: sale.taxJurisdictionCountry,
    taxJurisdictionState: sale.taxJurisdictionState,
    taxJurisdictionCounty: sale.taxJurisdictionCounty,
    taxStatus: sale.taxStatus,
    taxExempt: sale.taxExempt,
    taxExemptReason: sale.taxExemptReason,
    taxExemptionReference: sale.taxExemptionReference,
    taxCalculatedAt: sale.taxCalculatedAt?.toISOString() ?? null,
...
export async function testTargetDiscoveryUrl(url: string) {
  validateDiscoverySourceUrl("Target", url);
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "PokeRestockRadar/0.3 private-safe-discovery-test (+review-before-watch)"
    }
  });
...
export async function createInventoryItem(
  ...
  const item = await prisma.inventoryItem.create({
    data: {
      ...
      taxCategory: linkedInput.taxCategory,
      stripeTaxCode: linkedInput.stripeTaxCode,
      taxableOverride: linkedInput.taxableOverride,
...
export async function updateInventoryItem(
  ...
  await prisma.inventoryItem.update({
    ...
    data: {
      ...
      taxableOverride: input.taxableOverride,
      setName: input.setName,
      productId: input.productId,
      cardId: input.cardId,
      cost: nextCost,
...
function maskedPosEmail(value: string | null | undefined) {
...
function maskedPosPhone(value: string | null | undefined) {
  const digits = value?.replace(/\D/g, "") ?? "";
  return digits.length >= 4 ? `***-***-${digits.slice(-4)}` : digits ? "***" : null;
}

function maskedPosPaymentReference(value: string | null | undefined) {
  const trimmed = value?.trim();
  if (!trimmed) return null;
...
function posSaleFingerprint(input: {
  ...
  return posTaxCartFingerprint({
    ...
    profile: {
      ...
      profileEnabled: input.settings.tax.posTaxEnabled,
...
type PosSaleRecordWithItem = Prisma.InventorySaleGetPayload<{
  include: { inventoryItem: { include: typeof inventoryItemInclude }; user: { select: { name: true } } };
}>;
...
async function receiptForExistingPosSale(
  ...
  const sales = await client.inventorySale.findMany({
    ...
    include: { inventoryItem: { include: inventoryItemInclude }, user: { select: { name: true } } },
  ...
  const persistedTaxSnapshot = sales.some((sale) => sale.taxCents !== null);
  const subtotalCents = persistedTaxSnapshot
  ...
  const total = persistedTaxSnapshot
    ? centsToMoney(sales.reduce((sum, sale) => sum + (sale.totalCents ?? 0), 0))
    : noteTotal ?? roundPosMoney(centsToMoney(taxableSubtotalCents) + tax);
  const stateTaxCents = persistedTaxSnapshot ? sales.reduce((sum, sale) => sum + (sale.stateTaxCents ?? 0), 0) : taxCents;
  const countySurtaxCents = persistedTaxSnapshot ? sales.reduce((sum, sale) => sum + (sale.countySurtaxCents ?? 0), 0) : 0;
  const combinedRateBasisPoints = firstSale.combinedRateBasisPoints ?? (taxableSubtotalCents > 0 ? Math.round((taxCents * 10_000) / taxableSubtotalCents) : 0);
  const rewardSummary = await rewardSummaryForPosSaleReference(saleReference, client);
  const refundedAmount = roundPosMoney(sales.reduce((sum, sale) => sum + (sale.refundedAmount ?? 0), 0));
  const refundedTax = sales.some((sale) => sale.taxCents === null)
    ? null
    : centsToMoney(sales.reduce((sum, sale) => sum + (sale.refundedTaxCents ?? 0), 0));
  const refundedMerchandise = refundedTax === null ? null : roundPosMoney(Math.max(0, refundedAmount - refundedTax));
  const netTotal = roundPosMoney(Math.max(0, total - refundedAmount));
  const refundStatus = refundedAmount <= 0 ? null : netTotal <= 0 ? "refunded" : "partially_refunded";
  return {
    saleReference,
    paymentMethod,
    paymentMethodLabel: posPaymentMethodLabel(paymentMethod),
    paymentReference: maskedPosPaymentReference(firstSale.paymentReference),
    customerLinked: Boolean(firstSale.customerAccountId),
    customerEmail: maskedPosEmail(firstSale.customerEmail),
    customerPhone: maskedPosPhone(firstSale.customerPhone),
    ...
    tax,
    total,
    taxRate: combinedRateBasisPoints / 10_000,
    stateTax: centsToMoney(stateTaxCents),
    countySurtax: centsToMoney(countySurtaxCents),
    taxProvider: firstSale.taxProvider ?? "historical_unknown",
    taxStatus: firstSale.taxStatus ?? "not_recorded",
    taxExempt: firstSale.taxExempt ?? false,
    taxExemptReason: firstSale.taxExemptReason,
    taxJurisdiction: {
      country: firstSale.taxJurisdictionCountry ?? "US",
      state: firstSale.taxJurisdictionState ?? "FL",
      county: firstSale.taxJurisdictionCounty
    },
    cashierName: firstSale.user?.name ?? "Cashier unavailable",
    registerLabel: null,
    refundStatus,
    refundedAmount,
    refundedMerchandise,
...
async function createPosInventorySaleLine(
  ...
  const createdSale = await tx.inventorySale.create({
    data: {
      inventoryItemId: item.id,
      userId: currentUser.id,
      ...
      combinedRateBasisPoints: sale.taxStatus === "not_recorded" ? null : sale.combinedRateBasisPoints,
      taxJurisdictionCountry: sale.taxStatus === "not_recorded" ? null : sale.taxJurisdictionCountry,
      taxJurisdictionState: sale.taxStatus === "not_recorded" ? null : sale.taxJurisdictionState,
      taxJurisdictionCounty: sale.taxStatus === "not_recorded" ? null : sale.taxJurisdictionCounty,
      taxStatus: sale.taxStatus,
      taxExempt: sale.taxExempt,
      taxExemptReason: sale.taxExemptReason,
      taxExemptionReference: sale.taxExemptionReference,
...
function legacyManualFallbackActive(settings: StorefrontSettingsDTO, now = Date.now()) {
  const features = settings.tax.features;
  if (features.posTaxModeConflict) throw new Error("POS Stripe Tax and the legacy manual fallback cannot be active simultaneously.");
  const expiresAt = settings.tax.legacyManualTaxFallbackExpiresAt ? Date.parse(settings.tax.legacyManualTaxFallbackExpiresAt) : Number.NaN;
  return Boolean(features.manualTaxFallbackEnabled && !features.posSalesTaxEnabled &&
    settings.tax.legacyManualTaxFallbackEnabled &&
    settings.tax.legacyManualTaxFallbackAcknowledgedAt &&
    settings.tax.legacyManualTaxFallbackIncidentReason &&
...
export async function quotePosSaleTax(
  ...
  const stripeTaxEnabled = taxFeatures.posSalesTaxEnabled;
  const manualFallbackActive = legacyManualFallbackActive(settings);
  const manualFallbackMisconfigured = taxFeatures.manualTaxFallbackEnabled && Boolean(settings.tax.legacyManualTaxFallbackEnabled) && !manualFallbackActive;
  const misconfigured = (stripeTaxEnabled && !settings.tax.posTaxEnabled) || manualFallbackMisconfigured;
  const posLocation = stripeTaxEnabled ? await resolveTaxLocation(currentUser.id, "pos") : null;
  const shipFromLocation = stripeTaxEnabled ? await resolveTaxLocation(currentUser.id, "ship_from") : null;
  const storeAddress = stripeTaxEnabled ? (posLocation ? taxLocationAddress(posLocation) : verifiedStoreTaxAddress(settings)) : null;
  ...
  const calculation = stripeTaxEnabled && !misconfigured && storeAddress
    ...
    : null;
  const manualCalculation = manualFallbackActive ? calculateConfiguredPosTax({
    ...
    taxableSubtotalCents: lineAmounts.filter((line) => line.taxable).reduce((sum, line) => sum + line.netCents, 0),
    exempt: taxExempt,
    profile: {
      country: settings.tax.storeCountry,
      state: settings.tax.storeState,
      county: settings.tax.storeCounty,
      stateRateBasisPoints: settings.tax.stateRateBasisPoints,
      countyRateBasisPoints: settings.tax.countyRateBasisPoints,
  ...
  const taxStatus: PosTaxQuoteDTO["taxStatus"] = taxExempt ? "exempt" : misconfigured || blockedZero ? "misconfigured" : calculation || manualCalculation ? "collected" : "not_recorded";
  const providerStatus: PosTaxQuoteDTO["providerStatus"] = calculation?.providerStatus ?? (manualCalculation ? "calculated" : "disabled");
  const reason = misconfigured
    ? "The selected server-side tax mode is incomplete or expired. An admin must review Tax Settings."
    : blockedZero
      ...
      : calculation
        ...
        : manualCalculation
          ? "Legacy emergency fallback calculated from the saved incident snapshot. Cashiers cannot select or change this mode."
  ...
  const fingerprint = posSaleFingerprint({
    ...
    settings,
    runtimeEnabled: stripeTaxEnabled || manualFallbackActive
  });
  ...
  const taxCents = calculation?.taxCents ?? manualCalculation?.taxCents ?? 0;
  return {
    ...quoteToken,
    merchandiseSubtotal: centsToMoney(merchandiseSubtotalCents),
    discount: centsToMoney(discountCents),
    shipping: centsToMoney(shippingCents),
    taxableSubtotal: centsToMoney(calculation?.taxableSubtotalCents ?? manualCalculation?.taxableSubtotalCents ?? 0),
    stateTax: centsToMoney(calculation?.taxCents ?? manualCalculation?.stateTaxCents ?? 0),
    countySurtax: centsToMoney(manualCalculation?.countySurtaxCents ?? 0),
    tax: centsToMoney(taxCents),
    total: centsToMoney(merchandiseSubtotalCents - discountCents + shippingCents + taxCents),
    combinedRateBasisPoints: manualCalculation?.combinedRateBasisPoints ?? 0,
    taxStatus,
    providerStatus,
    ...
    effectiveAt: manualCalculation ? settings.tax.effectiveAt : null,
    sourceNote: calculation ? "Stripe Tax Calculations API" : manualCalculation ? settings.tax.sourceNote : null,
    locationName: posLocation?.name ?? (stripeTaxEnabled || manualCalculation ? "Legacy store profile" : null)
  };
}
...
export async function createPosSale(
  ...
  const taxFeatures = taxFeatureConfig();
  legacyManualFallbackActive(settings);
  ...
  const receipt = await runRewardSerializableTransaction(async (tx) => {
    ...
    const manualFallbackActive = legacyManualFallbackActive(activeSettings);
    ...
    if (input.selectedCustomerAccountId && customerMatch.customerAccountId !== input.selectedCustomerAccountId) {
      throw new Error("Selected customer changed or is no longer available. Refresh the tax calculation before completing the sale.");
    }
    const fingerprint = posSaleFingerprint({
      currentUserId: currentUser.id,
      idempotencyKey: input.idempotencyKey,
      ...
      items: lines.map((line) => ({
        ...
        originalUnitPriceCents: moneyToCents(line.originalUnitPrice),
        adjustedUnitPriceCents: moneyToCents(line.adjustedUnitPrice),
        discountReason: line.discountReason,
        taxable: line.taxable,
        taxCategory: line.taxCategory
      })),
      settings: activeSettings,
      runtimeEnabled: activeTaxFeatures.posSalesTaxEnabled || manualFallbackActive
    });
    ...
    const manualStateTaxByLine = allocateCentsByWeight(manualCalculation?.stateTaxCents ?? 0, lineTaxableSubtotalCents);
    const manualCountyTaxByLine = allocateCentsByWeight(manualCalculation?.countySurtaxCents ?? 0, lineTaxableSubtotalCents);
    ...
    const totals = {
      ...
      discountCents,
      taxableSubtotalCents: calculation?.taxableSubtotalCents ?? manualCalculation?.taxableSubtotalCents ?? lineTaxableSubtotalCents.reduce((sum, value) => sum + value, 0),
      stateTaxCents: calculation?.taxCents ?? manualCalculation?.stateTaxCents ?? 0,
      countySurtaxCents: manualCalculation?.countySurtaxCents ?? 0,
      taxCents: calculation?.taxCents ?? manualCalculation?.taxCents ?? 0,
      totalCents: merchandiseSubtotalCents - discountCents + shippingCents + (calculation?.taxCents ?? manualCalculation?.taxCents ?? 0),
      combinedRateBasisPoints: manualCalculation?.combinedRateBasisPoints ?? 0
    };
    const linesWithTax = lines.map((line, index) => {
      ...
      const stateTaxCents = calculation ? merchandiseTaxCents + shippingTaxForLine : manualStateTaxByLine[index] ?? 0;
      const countySurtaxCents = calculation ? 0 : manualCountyTaxByLine[index] ?? 0;
      ...
      return {
        ...
        discountCents: lineDiscountCents[index] ?? 0,
    ...
    const taxProvider = taxExempt ? "exempt" : posTaxEnabled ? "stripe_tax" : manualFallbackActive ? "configured_pos_rate" : "historical_unknown";
    const taxStatus = taxExempt ? "exempt" : posTaxEnabled || manualFallbackActive ? "collected" : "not_recorded";
    const taxRateSnapshot = JSON.stringify({
      provider: posTaxEnabled ? "stripe_tax" : manualFallbackActive ? "configured_pos_rate" : "historical_unknown",
      ...
      stateRateBasisPoints: manualFallbackActive ? activeSettings.tax.stateRateBasisPoints : null,
      countyRateBasisPoints: manualFallbackActive ? activeSettings.tax.countyRateBasisPoints : null,
      incidentExpiresAt: manualFallbackActive ? activeSettings.tax.legacyManualTaxFallbackExpiresAt : null
    });
    ...
    if (taxExempt) {
      await tx.taxAdjustment.create({
        data: {
          idempotencyKey: `tax:pos-exemption:${saleReference}`,
          channel: "pos",
          adjustmentType: "exemption",
          inventorySaleId: createdSaleIds[0] ?? null,
          saleReference,
          reason: input.taxExemptReason?.trim() ?? null,
          createdByUserId: currentUser.id,
          metadataJson: JSON.stringify({
            reference: input.taxExemptionReference?.trim() ?? null,
            note: input.taxExemptionNote?.trim() ?? null,
            customerAssociated: Boolean(customerMatch.customerAccountId),
    ...
    const rewardAward = await awardRewardsForCompletedPosSale(
      {
        customerAccountId: customerMatch.customerAccountId,
        saleReference,
        eligibleSubtotalCents: totals.subtotalCents - totals.discountCents,
        taxCentsExcluded: totals.taxCents,
    ...
    return {
      ...
      paymentMethod,
      paymentMethodLabel,
      paymentReference: maskedPosPaymentReference(paymentReference),
      customerLinked: Boolean(customerMatch.customerAccountId),
      customerEmail: maskedPosEmail(customerMatch.customerEmail),
      customerPhone: maskedPosPhone(customerMatch.customerPhone),
      customerMatchMethod: customerMatch.customerMatchMethod,
      rewardsEligible: customerMatch.rewardsEligible,
      rewardStatus,
      rewardPointsEarned,
      ...
      refundedTax: 0,
      netTotal: centsToMoney(totals.totalCents),
      refundedAt: null,
      itemCount: lines.reduce((sum, line) => sum + line.quantity, 0),
...
export async function refundPosSale(
  ...
  const remainingRefundableCents = sales.reduce((sum, sale) => {
  ...
  await runTaxRefundTransaction(async (tx) => {
    ...
    if (lockedSales.every((sale) => sale.refundStatus === "refunded")) {
      throw new Error("This POS sale has already been fully refunded.");
    }

    const remainingLineCents = lockedSales.map((sale) => Math.max(
      0,
    ...
    for (const [index, sale] of lockedSales.entries()) {
      ...
      const originalTotalCents = sale.totalCents ?? moneyToCents(sale.grossSale);
      const originalTaxCents = sale.taxCents;
      const previousRefundCents = moneyToCents(sale.refundedAmount ?? 0);
      const nextRefundCents = Math.min(originalTotalCents, previousRefundCents + refundAmountCents);
      const previousRefundedTaxCents = sale.refundedTaxCents ?? (originalTaxCents === null ? null : 0);
      const nextRefundedTaxCents = originalTaxCents === null
        ? null
        : cumulativeRefundedTaxCents({ originalTotalCents, originalTaxCents, cumulativeRefundedAmountCents: nextRefundCents });
      const refundedTaxDeltaCents = nextRefundedTaxCents === null
        ? 0
        : Math.max(0, nextRefundedTaxCents - (previousRefundedTaxCents ?? 0));
      cumulativeEligibleRefundCents += Math.max(0, refundAmountCents - refundedTaxDeltaCents);
      const lineFullyRefunded = nextRefundCents >= originalTotalCents;
      const restoredQuantity = input.restoreInventory && lineFullyRefunded
        ? Math.max(0, sale.quantitySold - (sale.refundRestockedQuantity ?? 0))
        : 0;
      const refundAuditNote = posRefundLineNote({
        saleReference: normalizedReference,
        reasonLabel,
        refundedAmount: centsToMoney(refundAmountCents),
      ...
      await tx.inventorySale.update({
        where: { id: sale.id },
        data: {
          refundStatus: lineFullyRefunded ? "refunded" : "partially_refunded",
          refundedAmount: centsToMoney(nextRefundCents),
          refundedTaxCents: nextRefundedTaxCents,
          taxStatus: originalTaxCents === null ? sale.taxStatus : nextRefundedTaxCents === originalTaxCents ? "refunded" : "partially_refunded",
          refundedAt,
          refundReason,
          refundNote: note,
      ...
      await tx.taxAdjustment.create({
        data: {
          idempotencyKey: `tax:pos-refund:${idempotencyKey}:${sale.id}`,
          channel: "pos",
    ...
    await reverseRewardsForPosSale(
      {
        ...
        reason: "refund",
        idempotencyKey,
        cumulativeEligibleRefundCents: lockedSales.reduce((sum, sale) => {
          const refunded = moneyToCents(sale.refundedAmount ?? 0);
          const refundedTax = sale.refundedTaxCents ?? 0;
          return sum + Math.max(0, refunded - refundedTax);
        }, 0) + cumulativeEligibleRefundCents,
        fullyRefunded: requestedRefundCents === remainingLineCents.reduce((sum, amount) => sum + amount, 0)
      },
      tx
    );
  });
