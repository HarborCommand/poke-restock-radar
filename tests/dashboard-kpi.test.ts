...
function sale(overrides: Partial<InventorySaleDTO> = {}): InventorySaleDTO {
  const base: InventorySaleDTO = {
    ...
    refundedTaxCents: null,
    customerAccountId: null,
    customerEmail: null,
    customerPhone: null,
    customerMatchMethod: null,
...
test("storefront publishing and distributor readiness workflow is wired", () => {
  ...
  assert.match(client, /homepageMerchandisingSections/);
  ...
  assert.ok(client.indexOf("<HomepageAccountCta settings={settings} signedIn={accountSignedIn} />") < client.indexOf("homepageSections.map"));
  assert.ok(client.indexOf("<HomepageGrabbyTip />") > client.indexOf("homepageSections.map"));
  assert.ok(client.indexOf("<HomepageGrabbyTip />") < client.indexOf("<h2>Shop By Category</h2>"));
  assert.ok(client.indexOf("<h2>Shop By Category</h2>") > client.indexOf("homepageSections.map"));
...
test("GameDayGrabs About and Policies pages use current customer policy copy", () => {
  ...
  assert.match(policiesPage, /Reward earning is not currently active/);
  assert.match(policiesPage, /eligible product purchases may receive points/);
  assert.match(policiesPage, /Points may start pending after payment/);
  assert.match(policiesPage, /may become available after fulfillment, pickup, or the configured pending period/);
  assert.match(policiesPage, /Shipping, taxes, refunds, discounts, canceled orders, and test\/smoke orders will not count toward points/);
  assert.match(policiesPage, /Refunded or canceled orders can reverse points/);
  assert.match(policiesPage, /Redemption is coming soon/);
...
test("admin health exposes build version and cache refresh controls", () => {
  ...
  assert.match(serviceWorker, /poke-radar-sw-2026-08-28-pos-current-sale-scroll-v1/);
