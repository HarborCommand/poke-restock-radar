...
test("GameDayGrabs About and Policies pages use current customer policy copy", () => {
  const aboutPage = fs.readFileSync(new URL("../src/app/about/page.tsx", import.meta.url), "utf8");
  const policiesPage = fs.readFileSync(new URL("../src/app/policies/page.tsx", import.meta.url), "utf8");
  const shippingPage = fs.readFileSync(new URL("../src/app/policies/shipping/page.tsx", import.meta.url), "utf8");
  const returnsPage = fs.readFileSync(new URL("../src/app/policies/returns/page.tsx", import.meta.url), "utf8");
  ...
  const trust = fs.readFileSync(new URL("../src/lib/storefront-trust.ts", import.meta.url), "utf8");
  const routing = fs.readFileSync(new URL("../src/lib/storefront-routing.ts", import.meta.url), "utf8");
  const combined = `${aboutPage}\n${policiesPage}\n${shippingPage}\n${returnsPage}\n${privacyPage}\n${termsPage}\n${trust}`;

  assert.match(aboutPage, /About GameDayGrabs/);
  assert.match(aboutPage, /independent online collectibles shop/);
  assert.match(aboutPage, /stocked inventory/);
  assert.match(aboutPage, /listed, packed, and fulfilled by GameDayGrabs/);
  assert.match(aboutPage, /No Affiliation Claims/);
  assert.match(aboutPage, /Email \$\{contactEmail\} for help with an order or product question/);

  assert.match(policiesPage, /Policy Pages/);
  assert.match(policiesPage, /storefrontPolicyLinks/);
  assert.match(policiesPage, /Shipping Summary/);
  assert.match(policiesPage, /Return & Refund Summary/);
  assert.match(policiesPage, /href="\/policies\/shipping"/);
  assert.match(policiesPage, /href="\/policies\/returns"/);
  assert.match(policiesPage, /href="\/privacy"/);
  assert.match(policiesPage, /href="\/terms"/);

  assert.match(trust, /Carriers and Services/);
  assert.match(trust, /Shipping costs are shown before payment/);
  assert.match(trust, /Lost or Delayed Packages/);
  assert.match(trust, /Return Window/);
  assert.match(trust, /All sealed trading card products/);
  assert.match(trust, /buyer-remorse returns/);
  assert.match(trust, /damaged, incorrect, missing, or materially different/);
  assert.match(trust, /photos of the package/);
  assert.match(trust, /photos of the product condition/);
  assert.match(trust, /photos of the shipping label/);
  assert.match(trust, /replacement, refund, partial refund/);
  assert.match(trust, /Opened, searched, resealed, tampered-with/);
  assert.match(trust, /Return Shipping/);
  assert.match(trust, /condition received/);

  assert.match(shippingPage, /shippingPolicySections/);
  assert.match(shippingPage, /order status page/);
  assert.match(returnsPage, /returnPolicySections/);
  assert.match(returnsPage, /Start an Order Issue/);
  ...
  assert.match(privacyPage, /Customer information is used for checkout, fulfillment, and support/);
  assert.match(termsPage, /termsSections/);
  assert.match(trust, /Store Use/);
  assert.match(trust, /Pricing, Taxes, and Shipping/);
