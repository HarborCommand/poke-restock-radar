...
test("route and UI keep stock adjustments admin-only, same-origin, idempotent, and mobile-safe", () => {
  ...
  assert.match(panel, /<small>Resulting<\/small>/);
  ...
  assert.match(panel, /Unit cost per unit \(USD\) \*/);
  assert.match(panel, /Whole units only/);
  assert.match(panel, /Stored as integer cents/);
  assert.match(panel, /maxLength=\{300\}/);
  assert.match(panel, /const \[noteLength, setNoteLength\] = useState\(0\);/);
  assert.match(panel, /setNoteLength\(event\.currentTarget\.value\.length\)/);
  assert.match(panel, /\{noteLength\} \/ 300/);
  assert.match(panel, /inventory-adjustment-summary-top/);
  assert.match(panel, /inventory-adjustment-stock-basis-card/);
  assert.match(panel, /This will create a new FIFO lot/);
  assert.match(panel, /This will consume existing stock using FIFO/);
  ...
  assert.match(history, /inventory-adjustment-history-card \$\{positive \? "is-add" : "is-remove"\}/);
  assert.match(history, /inventory-adjustment-history-date/);
  assert.match(history, /inventory-adjustment-history-body/);
  assert.match(history, /inventory-adjustment-delta-badge/);
  assert.match(history, /inventory-adjustment-note-indicator/);
  assert.match(history, /const positive = adjustment\.quantityDelta >= 0;/);
  assert.match(history, /positive \? "\+" : "-"/);
  ...
  assert.match(css, /body \.inventory-quick-adjustment-sheet \{[\s\S]*width: min\(1260px, 100%\);/);
  assert.match(css, /body \.product-workspace-stats \{[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\);/);
  assert.match(css, /body \.product-workspace-section-tabs button.active \{[\s\S]*border-color: #16a34a;/);
  assert.match(css, /body \.inventory-adjustment-layout \{[\s\S]*grid-template-columns: minmax\(0, 1\.62fr\) minmax\(380px, 1fr\);/);
  ...
  assert.match(css, /body \.inventory-adjustment-summary-flow \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto minmax\(0, 1fr\) auto minmax\(0, 1fr\);/);
  assert.match(css, /body \.inventory-adjustment-stock-basis-card \{[\s\S]*display: flex;/);
  assert.match(css, /body \.inventory-adjustment-history\.compact article\.inventory-adjustment-history-card \{[\s\S]*grid-template-columns: 42px minmax\(0, 1fr\) auto;/);
  assert.match(css, /body \.inventory-adjustment-history-meta span \{[\s\S]*white-space: nowrap;[\s\S]*text-overflow: ellipsis;/);
  assert.match(css, /body \.inventory-adjustment-main-inputs \{[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);/);
  assert.match(css, /body \.inventory-adjustment-note \{[\s\S]*min-height: 82px;[\s\S]*resize: vertical;/);
  assert.match(css, /@media \(max-width: 980px\)\s*\{[\s\S]*body \.inventory-adjustment-layout \{[\s\S]*grid-template-columns: minmax\(0, 1fr\);/);
  assert.match(css, /@media \(max-width: 980px\)\s*\{[\s\S]*body \.inventory-adjustment-summary-card \{[\s\S]*display: contents;/);
  assert.match(css, /@media \(max-width: 760px\)\s*\{[\s\S]*body \.product-workspace-mobile-back \{[\s\S]*display: inline-grid;/);
  assert.match(css, /@media \(max-width: 760px\)\s*\{[\s\S]*body \.inventory-adjustment-actions \{[\s\S]*grid-template-columns: minmax\(0, 0\.9fr\) minmax\(0, 1\.15fr\);[\s\S]*safe-area-inset-bottom/);
