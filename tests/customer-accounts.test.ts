...
test("customer account UI polish keeps account creation optional and mobile-safe", () => {
  ...
  assert.match(accountLogin, /copy\.loginFeatureLabel/);
  assert.match(accountLogin, /copy\.loginFeatureCopy/);
  assert.match(accountComponents, /rewardsEnabled \? "Rewards" : "Future Rewards"/);
  assert.match(accountComponents, /rewardsEnabled[\s\S]*"Earn points on eligible purchases\. Redemption coming soon\."[\s\S]*: "Be ready when rewards reopen\."/);
  ...
  assert.match(accountLogin, /copy\.loginPillCopy/);
  assert.match(accountComponents, /rewardsEnabled[\s\S]*"Earn points on eligible purchases\. Redemption coming soon\."[\s\S]*: "Reward earning is currently paused\."/);
  ...
  assert.match(accountComponents, /copy\.signedInScope/);
  assert.match(accountComponents, /signedInScope: rewardsEnabled \? "orders, rewards, saved addresses, and support" : "orders, future rewards, saved addresses, and support"/);
  ...
  assert.match(accountDashboard, /copy\.dashboardPanelCopy/);
  assert.match(accountComponents, /dashboardPanelCopy: rewardsEnabled[\s\S]*"Earn points on eligible purchases\. Redemption coming soon\."[\s\S]*: "Reward earning is not currently active\. Your account is ready for future rewards\."/);
  assert.match(accountComponents, /dashboardPointsCopy: rewardsEnabled[\s\S]*"Earning active\. Redemption coming soon"[\s\S]*: "Redemption coming soon"/);
  ...
  assert.match(accountComponents, /redemptionEnabled \? "Rewards active" : "Redemption coming soon"/);
...
test("customer rewards page shows balance activity and redemption coming soon", () => {
  ...
  assert.match(accountRewards, /copy\.spotlightStatus/);
  assert.match(accountRewards, /copy\.checkoutImpactCopy/);
  assert.match(components, /redemptionEnabled \? "Rewards active" : "Redemption coming soon"/);
  ...
  assert.match(accountRewards, /copy\.spotlightCopy/);
  assert.match(accountRewards, /copy\.rulesIntro/);
  assert.match(accountRewards, /copy\.rules\.map/);
  assert.match(components, /Reward earning is not currently active/);
  assert.match(components, /When earning resumes, eligible product purchases may receive points/);
  assert.match(components, /Eligible product purchases may receive points/);
  assert.match(components, /Points may remain pending until shipped, picked up, or cleared/);
  assert.match(components, /Shipping, tax, discounts, canceled\/refunded items, and test\/smoke orders will not count toward points/);
  assert.match(components, /Refunds\/cancellations may reverse previously recorded points/);
  assert.match(components, /Points have no cash value/);
  ...
  assert.match(accountRewards, /copy\.emptyActivityCopy/);
  assert.match(components, /Reward earning is currently paused/);
...
test("success page and order confirmation include safe optional account rewards CTA", () => {
  ...
  assert.match(client, /Track earned and pending points in your account\. Earn points on eligible purchases\. Redemption coming soon/);
  assert.doesNotMatch(sourceSlice(client, "export function CheckoutSuccessClient"), /Reward earning is currently paused/);
  assert.match(client, /No account required/);
  assert.match(emailTemplates, /Create your GameDayGrabs account to track orders and rewards/);
  assert.match(emailTemplates, /Earn points on eligible purchases\. Redemption coming soon/);
