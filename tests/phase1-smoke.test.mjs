...
test("inventory PDF exports are available without replacing CSV exports", () => {
  const pdfRoute = join(root, "src", "app", "api", "radar", "inventory", "export-pdf", "route.ts");
  const pdfLib = join(root, "src", "lib", "inventory-pdf.ts");
  assert.ok(statSync(pdfRoute).isFile(), `missing inventory PDF route ${pdfRoute}`);
  assert.ok(statSync(pdfLib).isFile(), `missing inventory PDF helper ${pdfLib}`);

  const route = readFileSync(pdfRoute, "utf8");
  for (const phrase of ["requireUser", "mode", "client", "internal", "application/pdf", "attachment"]) {
    assert.match(route, new RegExp(phrase), `missing PDF route phrase ${phrase}`);
  }

  const helper = readFileSync(pdfLib, "utf8");
  for (const phrase of [
    "PDFDocument",
    "Inventory Catalog",
    "Internal Inventory Report",
    "Available Products",
    "Generated by Poke Radar",
    "Availability subject to change",
    "Product",
    "Avg Cost",
    "Profit/Loss"
  ]) {
    assert.match(helper, new RegExp(phrase), `missing PDF helper phrase ${phrase}`);
  }

  const app = readFileSync(join(root, "src", "components", "RadarApp.tsx"), "utf8");
  for (const phrase of ["Export Client PDF", "Export Internal PDF", "Catalog CSV", "Lots CSV", "Sales CSV", "P/L CSV"]) {
    assert.match(app, new RegExp(phrase), `missing inventory export UI phrase ${phrase}`);
  }
});
