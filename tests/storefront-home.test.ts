...
test("homepage merchandising UI renders category links and safe product-card links", () => {
  ...
  assert.match(client, /Create an account to track orders\$\{rewardsEnabled \? " and rewards" : " and future rewards"\}/);
  ...
  assert.match(client, /Track orders, saved addresses\$\{rewardsEnabled \? ", and rewards" : ", and future rewards"\} from your dashboard/);
  assert.match(client, /Shop New Arrivals/);
  assert.match(client, /Reward earning is currently paused\. Redemption coming soon/);
  assert.match(client, /Earn points on eligible purchases\. Redemption coming soon/);
  assert.match(client, /Earn points on eligible purchases\. Manage rewards from your account/);
